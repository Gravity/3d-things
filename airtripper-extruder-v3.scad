...
module extruder_block() {
	difference() {
		union() {
			...
			difference() {
				translate([0,0,22]) cylinder(8,r=7.5, $fn=50, center = true);
				translate([5.2,4,22]) rotate([0,0,45]) cube([10,22,10], center = true);
				translate([5.2,-4,22]) rotate([0,0,315]) cube([10,22,10], center = true);
				//#rotate([0,90,0]) translate([-17,0,-10]) cylinder(8,r=4, $fn=50);
			}
			difference() {
				translate([0,0,3]) cylinder(22,r1=15, r2=6.3, $fn=100);	// rev. 3
			...
			difference() {
				...
				union(){
					translate([-21.5,0,10]) cube([6,74,20], center = true);
					translate([-24,30,10]) rotate([90,90,0]) cylinder(60, r=1.75, $fn=25);
				}
				union() {

					// fixing plate cutout
					//translate([-20,0,12]) cube([6,30,18], center = true);
					translate([-20,37,0]) rotate([45,0,0]) cube([10,16,7], center = true);
					translate([-20,-37,0]) rotate([135,0,0]) cube([10,16,7], center = true);
					translate([-20,37,20]) rotate([135,0,0]) cube([10,16,7], center = true);
					translate([-20,-37,20]) rotate([45,0,0]) cube([10,16,7], center = true);
					// M3 bolt holes
					translate([-27,30.5,10]) rotate([0,90,0]) cylinder(10, r=1.75, $fn=25);
					translate([-27,-30.5,10]) rotate([0,90,0]) cylinder(10, r=1.75, $fn=25);

					translate([-18.4,0,20]) rotate([45,0,0]) cube([3,20,20], center = true);
				}
			} // End of fixing plate
		}
		union() {
			...
			translate([0,0,22]) cylinder(5,r=5.2, $fn=50, center = true);
			translate([0,0,27]) cylinder(8,r=4.2, $fn=50, center = true);
			translate([0,0,25.5]) cylinder(2,r1=5.2, r2=4.2, $fn=50, center = true);
			translate([0,0,10.5]) cylinder(8	,r=7.2, $fn=100);	// rev. 2
			translate([0,0,18.5]) cylinder(3.5,r1=7.2, r2=3.5, $fn=50);	// rev. 2
	...
	difference() {
		union() {
			translate([5.8,14,13]) cube([10,14,26], center = true);
			...
			translate([4.3,21,13]) cube([13,3,26], center = true);	// rev. 3
			translate([11.8,17.5,0]) rotate([0,0,90]) bsp_push_fit();
			translate([-4,11.2,17.1]) rotate([15,0,0]) cube([10,5,20], center = true);
		}
		union() {
			...
			translate([12,7.5,14]) rotate([0,0,35]) cube([8,4,28], center = true);	// rev. 3
			translate([15,13.55,17]) rotate([0,0,280]) cube([20,10,28], center = true);
			//translate([4.5,8.5,25]) cube([10,4,10], center = true);	// rev. 3

			translate([-4.7,15.5,12]) cube([11,3,12], center = true);
			translate([-4,9,27.5]) cube([11,6,3], center = true);
			translate([-4,6,24.5]) cube([11,2,4], center = true);

			difference(){
			translate([-0.7,9,26.5])
			linear_extrude(height = 4, center = true, convexity = 10, twist = 0)
			polygon(points=[[0,0],[10,0],[5,10]], paths=[[0,1,2]]);
			
			translate([0.8,9.9,26.5])
			linear_extrude(height = 4, center = true, convexity = 10, twist = 0)
			polygon(points=[[0,0],[6,0],[3,6]], paths=[[0,1,2]]);
			}
		}
	}
